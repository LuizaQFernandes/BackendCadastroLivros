...
SP_S_LIV_CADASTROLIVRO_NOME 'Louisa May Alcott'
...
--SET
SELECT @DATA =  CAD_DT_DATAINCLUSAO FROM LIV_CADASTROLIVRO WHERE CAD_ST_NOME = @NOME OR UPPER(TRIM(@NOME)) = UPPER(TRIM(CAD_ST_AUTOR))
SELECT @PRECO  =  CAD_RE_PRECO FROM LIV_CADASTROLIVRO WHERE CAD_ST_NOME = @NOME OR UPPER(TRIM(@NOME)) = UPPER(TRIM(CAD_ST_AUTOR))
SET @FRETE = 0
...
BEGIN
SELECT @FRETE = CAD_RE_PRECO * 0.30 FROM LIV_CADASTROLIVRO WHERE CAD_ST_NOME = @NOME OR UPPER(TRIM(@NOME)) = UPPER(TRIM(CAD_ST_AUTOR))
END
...
BEGIN
SELECT @FRETE = CAD_RE_PRECO * 0.20 FROM LIV_CADASTROLIVRO WHERE CAD_ST_NOME = @NOME OR UPPER(TRIM(@NOME)) = UPPER(TRIM(CAD_ST_AUTOR))
END
...
BEGIN
SELECT @FRETE = CAD_RE_PRECO * 0.10 FROM LIV_CADASTROLIVRO WHERE CAD_ST_NOME = @NOME OR UPPER(TRIM(@NOME)) = UPPER(TRIM(CAD_ST_AUTOR))
END

--SE O LIVRO FOI ADICIONADO mais de 20 dias atras
IF  DATEDIFF(DD, @DATA, CONVERT(date, getdate())) > 20
BEGIN
SELECT @FRETE = CAD_RE_PRECO * 0 FROM LIV_CADASTROLIVRO WHERE CAD_ST_NOME = @NOME OR UPPER(TRIM(@NOME)) = UPPER(TRIM(CAD_ST_AUTOR))
END
